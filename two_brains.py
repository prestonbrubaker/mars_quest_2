...
planets = [[0.5, 0.5, 0, 0 - 0.1 * 0.1 * 0.02 / 5 + 0.3 * 0.1 * 0.003 / 5, 0, 0, 0.01, 50 / 3, [255, 195, 11]]]
...
death_age = 800 # Age when agent first has chance of random death
random_death_c = .1  # Chance of random death each iteration once at death age
metabolism_rate = 80000
color_mutation_chance = 0.5
mutation_c = 0.35
...
p_mass_c = 1   # number of food units worth of mass to be added as a constant to player
...
def run_brain(index_in):
    ...
    for k in range(2):
        ...
        players[index_in][10][k][0][11] = players[index_in][10][k][0][68]  # memory

        players[index_in][10][k][0][12] = players[index_in][10][k][0][69]  # memory 2
        ...
        if len(info_grid[x_index][y_index]) > 1:
            ...
            players[index_in][10][k][0][18] = sigmoid(dx_and_dy[index_cl][0] * grid_c)
            players[index_in][10][k][0][19] = sigmoid(len(dists))
            players[index_in][10][k][0][21] = players[index_list[index_cl]][10][k][0][70]
        ...
        for a in range(22):
            ...
            for b in range(22):
                ...
                weight_index += 1
            players[index_in][10][k][0][a + 22] = relu(sum)

        players[index_in][10][k][0][44] = 1  # bias
        ...
        for a in range(22):
            sum = 0
            for b in range(23):
                sum += players[index_in][10][k][0][b + 22] * players[index_in][10][k][1][weight_index]
                weight_index += 1
            players[index_in][10][k][0][a + 45] = relu(sum)

        players[index_in][10][k][0][67] = 1  # bias
        ...
        for a in range(11):
            ...
            for b in range(68):
                sum += players[index_in][10][k][0][b] * players[index_in][10][k][1][weight_index]
                weight_index += 1
            players[index_in][10][k][0][a + 68] = sigmoid(sum)
        ...
        decision_temp_list = []
        for i in range(5):
            decision_temp_list.append(players[index_in][10][k][0][i + 73])
    ...
    for i in range(len(decision_list[0])):
        decision_list_cond.append( decision_list[0][i] * sigmoid(players[index_in][10][0][1][weight_from_last]) + decision_list[0][i] * sigmoid(players[index_in][10][1][1][weight_from_last]) )
...
def iterate_players():
    ...
    for i in range(len(players)):
        ...
        if players[i][9] > 200 and players[i][10][0][0][71] > .5 and can_reproduce and (i != player_id or i == player_id and not can_die) and players[i][12] >= min_reproduction_age:
            if players[i][10][0][0][71] > 1: # players[i][10][0][72]:
                duplicate_player(i)
            else:
...
def sexually_duplicate_player(index_in):
    ...
    if len(info_grid[x_index][y_index]) > 1:
        ...
        if players[mate_index][10][0][0][72] > 0.5 and players[mate_index][9] > 200:
            births_by_sex += 1
...
def write_stats():
    with open('statistics.txt', 'a') as file:
        stats = str(itC) + " " + str(len(players)) + " " + str(wall_collisions) + " " + str(sun_collisions) + " " + str(births_by_sex) + " " + str(births_by_not_sex) + " " + str(old_age_deaths) + " " + str(starvation_deaths)
        file.write(stats + "\n")
...
def draw_text():
    ...
    outputs = ["               BRAIN 1",
               "Memory 1: " + str(round(players[player_id][10][0][0][68], 5)),
               "Memory 2: " + str(round(players[player_id][10][0][0][69], 5)),
               "Communication to others: " + str(round(players[player_id][10][0][0][70], 5)),
               "Willingness to reproduce male/asexually: " + str(round(players[player_id][10][0][0][71], 5)),
               "Willingness to reproduce sexually: " + str(round(players[player_id][10][0][0][72], 5)),
               "Rotate Left: " + str(round(players[player_id][10][0][0][73], 5)),
               "Thrust Forward: " + str(round(players[player_id][10][0][0][74], 5)),
               "Rotate Right: " + str(round(players[player_id][10][0][0][75], 5)),
               "Thrust Backwards: " + str(round(players[player_id][10][0][0][76], 5)),
               "Attempt Reproduction >= 0.5: " + str(round(players[player_id][10][0][0][77], 5)),
               ...
               "Thrust  coefficient: " + str(round(sigmoid(players[player_id][10][0][0][74]), 5))
              ]
    ...
    outputs_2 = ["               BRAIN 2",
               "Memory 1: " + str(round(players[player_id][10][1][0][68], 5)),
               "Memory 2: " + str(round(players[player_id][10][1][0][69], 5)),
               "Communication to others: " + str(round(players[player_id][10][1][0][70], 5)),
               "Rotate Left: " + str(round(players[player_id][10][1][0][73], 5)),
               "Thrust Forward: " + str(round(players[player_id][10][1][0][74], 5)),
               "Rotate Right: " + str(round(players[player_id][10][1][0][75], 5)),
               "Thrust Backwards: " + str(round(players[player_id][10][1][0][76], 5)),
              "Attempt Reproduction >= 0.5: " + str(round(players[player_id][10][1][0][77], 5)),
...
while running:
    ...
    for i in range(len(players)):
        ...
        if command != None:
            sensitivity_p = players[player_id][10][0][0][74] * sensitivity
