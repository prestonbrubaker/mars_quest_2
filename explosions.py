...
def create_players(number_to_make):
    for i in range(number_to_make):
        ...
        for j in range(49):
            nodes.append(0)
        weights = []
        for j in range(660): # 10 reserved for other used, rest used for neural network
...
def run_brain(index_in):
    ...
    for k in range(2):
        ...
        players[index_in][10][k][0][11] = players[index_in][10][k][0][35]  # memory

        players[index_in][10][k][0][12] = players[index_in][10][k][0][36]  # memory 2
        ...
        if len(info_grid[x_index][y_index]) > 1:
            ...
            players[index_in][10][k][0][21] = players[index_list[index_cl]][10][k][0][37]
            players[index_in][10][k][0][22] = players[index_list[index_cl]][10][k][0][38]
        ...
        players[index_in][10][k][0][34] = 1  # bias



        # hidden + input to output: setting nodes 35 to 48 using nodes nodes 0-34
        ...
        for a in range(14):
            ...
            for b in range(35):
                sum += players[index_in][10][k][0][b] * players[index_in][10][k][1][weight_index]
                weight_index += 1
            players[index_in][10][k][0][a + 35] = sigmoid(sum)

        # 35 & 36 Are alloted to nodes 11 & 12 (memory)

        # 37 & 38 reserved as output to communicate out to other agents

        # 39 is willingness to reproduce asexually

        # 40 is willingness to reproduce sexually

        # 41 to 46 reserved for actions

        # 47 reserved for thrust factor

        decision_temp_list = []
        for i in range(6):
            decision_temp_list.append(players[index_in][10][k][0][i + 41])
...
def iterate_players():
    ...
    for i in range(len(players)):
        ...
        # Metabolism
        players[i][9] -= dt * metabolism_rate
        if players[i][9] < 0:
            players[i][9] = 0
        x = players[i][0]
        y = players[i][1]
...
def sexually_duplicate_player(index_in):
    ...
    if len(info_grid[x_index][y_index]) > 1:
        ...
        if players[mate_index][10][0][0][40] > 0.5 and players[mate_index][9] > 200:
            births_by_sex += 1

            food_transfered_father = players[index_in][9] * players[index_in][10][0][0][48] * 0.2
            players[index_in][9] -= food_transfered_father
            food_transfered_mother = players[mate_index][9] * players[index_in][10][0][0][48] * 0.8
            ...
            player_list[8] = -1
            player_list[9] = food_transfered_father + food_transfered_mother
            player_list[12] = 0
...
def duplicate_player(index_in):
    ...
    births_by_not_sex += 1
    food_transfered = players[index_in][9] * players[index_in][10][0][0][48]
