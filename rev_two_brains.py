...
def create_players(number_to_make):
    for i in range(number_to_make):
        ...
        for j in range(47):
            nodes.append(0)
        weights = []
        for j in range(580): # 10 reserved for other used, rest used for neural network
...
def add_food():
    for i in range(food_cycles):
        ...
        if dist > 0.43 or dist < 0.3 or (y > 0.475 and y < 0.525 and x < 0.5):
...
def run_brain(index_in):
    ...
    for k in range(2):
        ...
        # hidden + input to output: setting nodes 35 to 46 using nodes nodes 0-34
...
def draw_minimap():
    ...
    pygame.draw.circle(window, (255, 0, 0), (minimap_x + 0.5 * minimap_size, minimap_y + 0.5 * minimap_size), 0.32 * minimap_size, 2)
...
def draw_text():
    ...
    outputs = ["               BRAIN 1",
               ...
               "Rest: " + str(round(players[player_id][10][0][0][45], 5)),
               ...
               "Thrust coefficient: " + str(round(sigmoid(players[player_id][10][0][0][46]), 5)),
    ...
    outputs_2 = ["               BRAIN 2",
               ...
               "Rest: " + str(round(players[player_id][10][1][0][45], 5)),
